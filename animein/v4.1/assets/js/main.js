                            ...
                            let getNewsEpisode = async (el) => {
                            		...
                            		if(typeof entry != "undefined" && entry != null && entry.length != null
&& entry.length > 0) {
                            			for(let {rel, href} of entry[0].link) {
                            				if(rel == "alternate") {
                            ...
                            window.addEventListener ? window.addEventListener('load', jonaLazyLoad, false) : window.attachEvent ? window.attachEvent('load', jonaLazyLoad) : window.onload = jonaLazyLoad, window.addEventListener ? window.addEventListener('load', jonaLazyLoad, false) : window.attachEvent ? window.attachEvent('load', jonaLazyLoad) : window.onload = jonaLazyLoad;
...
clearInterval(interval);
                            ...
                            let post_ainfo = document.querySelector('.jona_animeinfo .item-post .dataJSON');
                                try {
                                    let doc = new DOMParser().parseFromString('<div class="data">'+post_ainfo.innerHTML+'</div>', "text/xml")
                                    let json = JSON.parse(doc.querySelector('.dataJSON').textContent);
                            ...
                            try {
                                let doc = new DOMParser().parseFromString('<div class="data">'+animepost.innerHTML+'</div>', "text/xml")
                                let {streaming, download} = JSON.parse(doc.querySelector('.dataJSON').textContent);
                                let img = doc.querySelector('.separator a img').getAttribute('src');
                                ...
                                let tabs = document.querySelector('.jona_animepost .etabs-list');
                                ...
                                if(download) {
                                    if(download.length > 0) {
                                        ...
                                        tab1.innerHTML = `<div class="dl"><h3>${title}</h3><div class="dl-list"></div></div>`;

                                        /* [Show url] */
                                        let dllist = document.querySelector('.dl .dl-list');
                                        for({quality, name, url} of download) {
                                ...
                                scripteps.onload = () => {scripteps.remove()};
                            } catch (e) {
                            ...
                            let ambil_mal = async (id) => {
                                ...
                                score.innerHTML = `<span class='title'>Score</span><div class="score_"><div class="info"><div class="star"><svg
									xmlns="http://www.w3.org/2000/svg" width="6rem" height="6rem" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="feather feather-star"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg><label>${json_get.data.score}</label></div>
                                    ${Object.keys(json_stat.data).map(k => k !== 'scores' ? `<div class="list"><label>${k.replace(/_/g,' ')}</label><span>${nFormatter(json_stat.data[k], 1)}</span></div>` : '').join('')}
                                    </div><div class="stats"><h3>Votes</h3>
                                        ${json_stat.data.scores.map(e => `<div class="stat s${e.score}"><span>${e.score}</span><div><span style="width:${e.percentage}%"></span><label>${nFormatter(e.votes, 1)}</label></div></div>`).join('')}
                                    </div></div>`;
                                ...
                                list_char.innerHTML = `<span class="title">Karakter & Staff</span><div class="karakter_">
                                    ${json_char.data.map(c => `<div class="daftar"><span class="role">${c.role}</span><div class="char"><img src="${c.character.images.jpg.image_url}"/><span>${c.character.name}</span></div><div class="staff">
                                            ${c.voice_actors.map(p => `<div class="person"><img src="${p.person.images.jpg.image_url}"/><span class="lang">${p.language}</span><span class="name">${p.person.name}</span></div>`).join('')}
                                        </div></div>`).join('')}
                                </div>`;
                                info.querySelector('.tab3').appendChild(list_char);
                            }

                            let episode_perid = ({feed}) => {
                                let {entry} = feed;
                                if(entry) {
                                    let show_eps = (e) => {
                                        let {content} = e;
                                        ...
                                        if(doc.querySelector('.dataJSON')) {
                                            ...
                                            } catch(e) {/*console.log('terdeteksi bukan JSON di episode_perid')*/}
                                        } else {
